...
-- cauce-channels integration tests exercise Row-Level Security through the real
-- application path (the webhook resolves configs via the V16 SECURITY DEFINER function and processes
-- under RLS). DML grants are applied by Flyway migration V10; this script only adds LOGIN and
-- the test password wired in AbstractChannelsIntegrationTest.
DO $$
BEGIN
