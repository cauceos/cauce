...
- **Audit trail (governance) — capture skeleton + hash chain landed.** V20 `audit_outbox`
  ...
  CUSTOMER's legal judgment). Still pending: **Family B** (tenancy/keys operations — create/
  revoke agents, keys, tenants), and activating the four reserved conduct hooks
  (`external.action` when a side-effect tool exists, `data.access` when a tool reaches external
  data, `human.escalation` when `escalateConversation` gets a caller, `reply.delivered` when
  delivery becomes transactional); **retention purge of DRAINED outbox rows** (raw payloads also
  live there until purged); and the **public verification surface** (endpoint + compliance
  report — the verifier is internal today).
