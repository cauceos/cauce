...
- `playground` — developer testing tool against a running Cauce instance (React 19 + Vite 7 +
  ...
  TOOL_CALL/TOOL_RESULT messages; the workspace tenant id is pasted by the user — the API
  has no "who am I" — and lives in the session context, memory-only). The remaining areas
  (tenants, agents, API keys, invocations, audit chain) are placeholders for the next units.
...
- **LLM usage accounting — capture landed** (V19 `llm_usage_records`: one immutable row per LLM
  ...
  deferrals: no query endpoint or aggregations, and **no cost materialization** — deliberate:
...
- **Audit trail (governance) — capture skeleton + hash chain landed.** V20 `audit_outbox`
  ...
  rows honestly reported, never backfilled). Remaining units: **signature** over the reserved
  `signature` column (`entry_hash` + `hash_scheme` are its stable target; it also closes the
  documented residual — a privileged attacker re-chaining the ENTIRE suffix and rewriting the
  head is undetectable by recomputation alone); **wiring real auditable events — Family A
  ...
  Bearer-authenticated, hierarchical — 404 outside visibility; it returns the verdict, the
  first-break sequence + public classification, the pre-chain count and a
  `verification_scope` naming what recomputation does and does not detect).
...
- **No key-introspection ("who am I") endpoint.** No authenticated response exposes the
  caller's tenant (the auth principal carries it, but no controller surfaces it), so API
  consumers — the playground included — must be told their tenant id out of band (it is in
  the bootstrap WARN line and the quickstart state file). A small introspection endpoint
  (caller tenant id + key id) is the natural future unit; it pairs with the deferred
  per-key actor attribution in governance.
- **`tool_content` is not exposed on the message DTO.** `MessageResponse` flattens tool
  messages to `content` only (tool name for TOOL_CALL, output text for TOOL_RESULT); the
  structured JSONB (`tool_call_id`, `input`, `is_error`) never reaches the wire, so clients
  cannot show tool arguments or error flags. Exposing it (nullable `tool_content` on the
  DTO) is a pure add when a consumer needs it — the playground's loop trace currently
  renders only what the wire carries.
