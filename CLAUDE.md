...
- `cauce-memory` — persistence: JPA entities, hand-written mappers, Spring Data repositories, `RlsContextAspect`, Flyway migrations (V1–V19 — all modules' migrations live here, incl. the messages `tool_content` jsonb column, the `ingest_idempotency_records` table, and the `llm_usage_records` ledger). Vector retrieval is planned (pgvector enabled, no code yet)
