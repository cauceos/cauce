...
- **Audit trail (governance) — capture skeleton + hash chain landed.** V20 `audit_outbox`
  ...
  payloads also live there until purged); and the **public verification surface** (endpoint +
  compliance report — the verifier is internal today).
...
- **Dashboard.** The product frontend (`frontend/cauce-dashboard`, Angular) does not exist.
  ...
  landed, the five remaining areas are placeholders. The playground also ships without a
