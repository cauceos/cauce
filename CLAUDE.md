...
- **Audit trail (governance) — capture skeleton + hash chain landed.** V20 `audit_outbox`
  ...
  head is undetectable by recomputation alone); **wiring real auditable events** from the
  loop/tenancy, under the INHERITED RESTRICTION recorded now: the append-only sink must NEVER
  store raw erasable sensitive content — erasable data lives in the mutable business tables
  (where deletion operates), the sink keeps only its `payload_hash` + non-sensitive metadata;
  append-only and the right to erasure only coexist that way. The chain's erasure compatibility
  is a TECHNICAL capability — the legal erasure verdict is always the customer's; **retention
  purge of DRAINED outbox rows** (raw payloads also live there until purged); and the **public
  verification surface** (endpoint + compliance report — the verifier is internal today).
